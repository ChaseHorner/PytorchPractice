...
def test_encoder_with_sentinel_concat():
    ...
    enc_1 = model.Encoder(configs.C3 + configs.S1, configs.C4, scale_size)

    input_tensor = torch.randn(1, configs.C3 + configs.S1, 256, 256)
    output_tensor = enc_1(input_tensor)

    assert output_tensor.shape == (1, configs.C4, 128, 128), \
        f"Expected shape {(1, configs.C4, 128, 128)}, but got {output_tensor.shape}"
    

## This one is really slow, so it's commented out for regular testing
'''
def test_encoder_start():
    in_channels = configs.LIDAR_IN_CHANNELS
    out_channels = configs.LIDAR_OUT_CHANNELS
    scale_size = 2
    batch_size = 1
    height, width = configs._LIDAR_SIZE
...
def test_encoder_chain_end():
    enc_1 = model.Encoder(configs.C6, configs.C7, scale_size = 2)
    enc_2 = model.Encoder(configs.C7, configs.C8, scale_size = 2)

    input_tensor = torch.randn(1, configs.C6, 32, 32)
    output_tensor_1 = enc_1(input_tensor)
    output_tensor_2 = enc_2(output_tensor_1)

    assert output_tensor_2.shape == (1, configs.C8, 8, 8), \
        f"Expected shape {(1, configs.C8, 8, 8)}, but got {output_tensor_2.shape}"
...
def test_encoder_chain_sentinel_concat():
    enc_1 = model.Encoder(configs.C2, configs.C3, scale_size = 5)
    enc_2 = model.Encoder(configs.C3 + configs.S1, configs.C4, scale_size = 2)

    input_tensor = torch.randn(1, configs.C2, 1280, 1280)
    output_tensor_1 = enc_1(input_tensor)
    ...
    assert output_tensor_2.shape == (1, configs.C4, 128, 128), \
        f"Expected shape {(1, configs.C4, 128, 128)}, but got {output_tensor_2.shape}"